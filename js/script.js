...
function cargarDesdeLocal() {
    ...
    actualizarListaPoligonos();
    // =========================================================
// 10. BLOQUE DE HERRAMIENTAS DE MEDICIÓN (Distancia y Área)
// =========================================================

// 1. Activar/Desactivar Medición de Distancia
document.getElementById('btn-modo-medicion').onclick = () => {
    modoMedicion = !modoMedicion;
    modoPoligono = false; // Apagamos área si estaba activa
    puntosTemp = [];
    const btn = document.getElementById('btn-modo-medicion');
    btn.innerText = modoMedicion ? "📏 Medir: ACTIVO (Haz 2 clics)" : "📏 Medir Distancia";
    btn.style.background = modoMedicion ? "#e67e22" : "#0078d4";
};

// 2. Activar/Desactivar Medición de Área
document.getElementById('btn-modo-poligono').onclick = () => {
    modoPoligono = !modoPoligono;
    modoMedicion = false; // Apagamos distancia si estaba activa
    puntosTemp = [];
    const btn = document.getElementById('btn-modo-poligono');
    btn.innerText = modoPoligono ? "⬢ Área: ACTIVO (Doble clic para cerrar)" : "⬢ Medir Área";
    btn.style.background = modoPoligono ? "#27ae60" : "#0078d4";
};

// 3. Lógica de clics en el mapa
map.on('click', (e) => {
    // Si estamos midiendo distancia
    if (modoMedicion) {
        puntosTemp.push(e.latlng);
        L.circleMarker(e.latlng, { radius: 4, color: '#e74c3c' }).addTo(map);

        if (puntosTemp.length === 2) {
            const dist = map.distance(puntosTemp[0], puntosTemp[1]);
            const linea = L.polyline(puntosTemp, { color: '#e74c3c', weight: 3, dashArray: '5, 5' }).addTo(map);
            
            linea.bindTooltip(`Distancia: ${dist.toFixed(2)} m`, {
                permanent: true, direction: "center", className: "etiqueta-medicion"
            }).openTooltip();

            puntosTemp = []; 
            modoMedicion = false;
            document.getElementById('btn-modo-medicion').style.background = "#0078d4";
            document.getElementById('btn-modo-medicion').innerText = "📏 Medir Distancia";
        }
    }

    // Si estamos dibujando un polígono para área
    if (modoPoligono) {
        puntosTemp.push(e.latlng);
        L.circleMarker(e.latlng, { radius: 4, color: '#2ecc71' }).addTo(map);

        if (puntosTemp.length > 1) {
            L.polyline([puntosTemp[puntosTemp.length - 2], puntosTemp[puntosTemp.length - 1]], {
                color: '#2ecc71', weight: 2
            }).addTo(map);
        }
    }
});

// 4. Cerrar Polígono y calcular Área (Doble clic)
map.on('dblclick', (e) => {
    if (modoPoligono && puntosTemp.length > 2) {
        const poligono = L.polygon(puntosTemp, {
            color: '#27ae60', fillColor: '#2ecc71', fillOpacity: 0.3
        }).addTo(map);

        // Cálculo de área (Requiere la librería GeometryUtil en el HTML)
        const areaM2 = L.GeometryUtil.geodesicArea(puntosTemp);
        const centro = poligono.getBounds().getCenter();

        // Mostrar etiqueta con Hectáreas
        L.marker(centro, { 
            icon: L.divIcon({ 
                className: 'etiqueta-area', 
                html: `<b>Área:</b><br>${(areaM2 / 10000).toFixed(4)} Ha` 
            }) 
        }).addTo(map);

        puntosTemp = [];
        modoPoligono = false;
        document.getElementById('btn-modo-poligono').style.background = "#0078d4";
        document.getElementById('btn-modo-poligono').innerText = "⬢ Medir Área";
    }
});
}